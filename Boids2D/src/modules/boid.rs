...
const ALIGNMENT_WEIGHT: f32 = 0.8; //1.0;
const SEPERATION_WEIGHT: f32 = 1.1; //1.0;
...
impl Boid {
    pub fn new(rng: &mut ThreadRng, window_size: Vector2<f32>) -> Boid {
        ...
        let forward = Vector2::new(
            math::to_radians(angle + 90.0).cos(),
    ...
    pub fn update(&mut self, window_dim: Vector2<f32>, delta_time: f32) {
        ...
        if self.local_boids.len() > 0 {
            ...
            let alignment_force = math::normalise_vector2(self.local_boids.average_dir_heading())
                * MAX_SPEED
                - self.velocity;
            let cohesion_force =
                math::normalise_vector2(offset_to_center) * MAX_SPEED - self.velocity;
            let seperation_force =
                math::normalise_vector2(self.local_boids.seperation()) * MAX_SPEED - self.velocity;
        ...
        let angle_dir = Vector2::new(
            250.0 * (math::to_radians(self.angle + 90.0)).cos() * delta_time,
    ...
    pub fn draw(&self, draw_calls: &mut Vec<DrawCall>) {
        ...
        Vector3::new(0, 0, 1)));*/
        draw_calls.push(DrawCall::draw_textured(
            self.pos,
            self.size,
            self.angle,
            self.texture.to_string(),
