...
impl SceneData {
    pub fn new(
        ...
        model_sizes: Vec<(String, Vector3<f32>)>,
        terrain_data: Vec<(String, Vec<Vec<f32>>)>,
    ) -> SceneData {
        SceneData {
            should_close: false,
            next_scene: false,
            ...
            controller: None, //Controller::new(),
            model_sizes,
            terrain_data,
...
pub trait Scene {
    ...
    fn add_model_size(
        ...
        reference: String,
        size: Vector3<f32>,
        terrain_data: Option<Vec<Vec<f32>>>,
    ) {
        println!("Name: {}, size: {:?}", reference, size);
        self.mut_data()
            .model_sizes
            .push((reference.to_string(), size));
        if let Some(terrain_data) = terrain_data {
            self.mut_data()
                .terrain_data
                .push((reference.to_string(), terrain_data));
        }
    }

    fn handle_event(&mut self, mut event: winit::event::Event<()>) {
        match event {
            winit::event::Event::WindowEvent { event: w_event, .. } => match w_event {
                ...
                winit::event::WindowEvent::CursorMoved {
                    ...
                    let mouse_pos = Vector2::new(
                        position.x as f32,
                        self.data().window_dim.y - position.y as f32,
                    );
                    ...
                    winit::event::DeviceEvent::MouseWheel { delta, .. } => {
                        match delta {
                            PixelDelta(scroll_delta) => {
                                println!(
                                    "Not used. Please contact Lilith@inet-sys.com: {}",
                                    scroll_delta.y
                                );
                            }
                            LineDelta(_x, y) => {
